...
for k,v in kube_config_dict.items(): #补全kube-config内容，其中token在调用时生成
    ...
    kube_config_dict[k]['kube']["clusters"] = [{'name': k,
                                                "cluster": {"certificate-authority-data": kube_config_dict[k]['certificate-authority-data'],
                                                             "server": kube_config_dict[k]['server']}
                                              }]
    kube_config_dict[k]['kube']['contexts']= [{"context": {"cluster": k,
                                                            "user": "sa"},
                                                "name": "context1"
                                            }]
    kube_config_dict[k]['kube']['users'] = [{"user": {"token": ''},
                                             "name": "sa"
                                            }]
